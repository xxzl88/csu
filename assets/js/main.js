(function () {
  ...
  results.innerHTML = visible.length
    ? visible.map((post, index) => {
        const serial = start + index + 1; // 连续编号
        return `<article class="topic-item">
          <div class="post-meta">
            <span class="post-index">${serial}</span>
            <span class="dot">·</span>
            <time>${escapeHtml(post.dateLabel)}</time>
            <span class="dot">·</span>
            <span>${escapeHtml(categories(post)[0])}</span>
          </div>
          <h3><a href="${escapeHtml(post.url)}">${escapeHtml(post.title)}</a></h3>
          <p>${escapeHtml(post.excerpt)}</p>
        </article>`;
      }).join('')
    : '<p class="loading">这个主题暂时还没有文章。</p>';
  pagination.innerHTML = total > 1
    ? `<button class="topic-filter" data-page="prev" ${page === 1 ? 'disabled' : ''}>←</button>
       <span>第 ${page} / ${total} 页</span>
       <button class="topic-filter" data-page="next" ${page === total ? 'disabled' : ''}>→</button>`
    : '';
